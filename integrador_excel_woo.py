...
def numero_seguro(valor, padrao=None):
    """
    Converte valores vindos do fornecedor com segurança.
    Evita erro quando a API retorna None, vazio ou texto inválido em campos numéricos.
    """
    try:
        if valor is None:
            return padrao
        ...
        if isinstance(valor, str):
            valor = valor.strip()
            if valor == "" or valor.lower() in ["none", "null", "undefined"]:
                return padrao
            # aceita formato brasileiro caso venha como texto: 1.234,56
            if "," in valor:
...
def inteiro_seguro(valor, padrao=None):
    n = numero_seguro(valor, padrao=None)
    if n is None:
        return padrao
    try:
        return int(n)
...
def enviar(prod, cache):
    ...
    estoque_fornecedor = inteiro_seguro(prod.get("stock"), 0)
    ...
    try:
        ...
        def processar(item):
            try:
                ...
                id_categoria = inteiro_seguro(detalhe.get("idcategoria"), 0)
                ...
                # 🔥 PREÇO E ESTOQUE COM BLINDAGEM
                # Se o fornecedor mandar None/vazio em preço ou saldo, não atualiza o produto.
                # Isso evita zerar preço/estoque por falha temporária da API.
                preco_raw = item.get("precovenda")
                preco = numero_seguro(preco_raw, None)

                if preco is None or preco <= 0:
                    log(f"⚠️ {sku} ignorado - preço inválido recebido do fornecedor: {preco_raw!r}")
                    return

                saldo_raw = item.get("saldo")
                saldo_fornecedor = inteiro_seguro(saldo_raw, None)

                if saldo_fornecedor is None:
                    log(f"⚠️ {sku} ignorado - saldo inválido recebido do fornecedor: {saldo_raw!r}")
                    return
